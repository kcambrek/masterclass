""" Plotting clusters with silhoutte analysis
 ...
 EXAMPLE
   ...
   [data, labels_true] = make_blobs(n_samples=750, centers=[[1, 1], [-1, -1], [1, -1], [-1, 1]], cluster_std=0.4,random_state=0)
   from DBSCAN import DBSCAN
   out = DBSCAN(data, showfig=0)
...
#        ax1.fill_betweenx(np.arange(y_lower, y_upper), 0, ith_cluster_silhouette_values, facecolor=getcolors[i], edgecolor=getcolors[i], alpha=0.7)
    ...
    color = cmap(cluster_labels.astype(float) / n_clusters)
