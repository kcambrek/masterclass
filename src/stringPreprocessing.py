...
import numpy as np
import pandas as pd
import strtricks as strtricks # Some string tricks
import re # For ngrams
...
#                string = re.sub(r'strvariant1, newchar, list(data))
        ...
        if len(alias[i])<=4:
            ...
            aliasGrams = strp.ngrams(alias[i],3)
        else:
            aliasGrams = strp.ngrams(alias[i],4)
        #end            
...
def delnumstring(data):
    ...
    #delnumstring = np.vectorize(strp.del_num_in_string)
