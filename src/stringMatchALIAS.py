""" This function links small ALIAS words to longer input strings based on EXACT and FUZZY similarity
	...
	[dist_matrix, bin_match] = stringMatchALIAS(data, alias, <optional>)
    v1.4
 ...
 OPTIONAL
   ...
   scoreOK=      : [Float]: [0,..,1] Strings are always included when it equals this score or being higher
                   [1] (default)
                   [1, 0.6, 0.9] # list of cutt-off scores. Must be length of ALIAS
 ...
 DESCRIPTION
   Detect groups of EXACT similar words

 EXAMPLE
   %reset -f
   import sys, os, importlib
   import stringMatchALIAS
   import pandas as pd
   import numpy as np
...
# EXAMPLE 1
   ...
   tic()
   [outDIST,outBIN] = stringMatchALIAS(data,alias)
   [outDIST,outBIN] = stringMatchALIAS(data,alias, scoreOK=0.9)
   [outDIST,outBIN] = stringMatchALIAS(data[0:150],data[0:150], scoreOK=0.9, clean='')
   ...
   [outDIST,outBIN] = stringMatchALIAS(data,alias,methodtype='FUZZY')
   ...
   [outDIST,outBIN] = stringMatchALIAS(data[0:500],data[0:500], methodtype='FUZZY', clean='complete')
...
import numpy as np
import pandas as pd
import showprogress as sp # Progressbar
from tictoc import tic, toc # Timer
import stringPreprocessing as strp # Current function
checkLen = np.vectorize(len) # To check length in arrays
from fuzzywuzzy import fuzz

#%%
def stringMatchALIAS(data, alias, remwords=[], minchar=2, maxchar=25, maxlen=200, scoreOK=1, clean=['complete'], methodtype=['complete'], showprogress=1):
	#%% DECLARATIONS
    outBIN  = []
    ...
    #%% INPUT CHECK
    if Param['showprogress']: print(">Input check..")

    data     = strp.typecheck(data, Param['maxlen'])
    alias    = strp.typecheck(alias, Param['maxlen'])
    remwords = strp.typecheck(remwords, Param['maxlen'])
    ...
    data = strp.stringRemWords(data, remwords)
    
    #%% Remove numbers in string
    if np.any(np.in1d(Param['clean'], ['complete','numeric'])):
        if Param['showprogress']: print(">String cleaning (removal of numbers)..")
        data  = strp.delnumstring(data)
        alias = strp.delnumstring(alias)
    #end

    #%% Cleaning, Lower/strip from DATA
#    if np.any(np.in1d(Param['clean'], ['complete','lower-strip'])):
    if Param['showprogress']: print(">String cleaning (lower/strip)..")
    data  = strp.stringClean(data)
    alias = strp.stringClean(alias)
    #end
    ...
    if np.any(np.in1d(Param['methodtype'], ['complete','EXACT-FAST'])):
        if Param['showprogress']: print(">Start EXACT matching (warming-up) [%d rows].." %(outDIST.shape[0]))
...
#    out = pd.DataFrame(index=range(0,len(data)), data=False, columns=alias)
    ...
    for i in range(0,len(alias)):
        ...
        if extensive==1 and np.any(np.in1d(Param['clean'], ['complete','alias-specific'])):
            ...
            [dataPR, aliasPR] = strp.stringPreprocessing(data, [alias[i]], sepchar=Param['sepchar'])
            # Now remove, words with min-max-size, this step MUST be 
            dataPR            = strp.stringMinMaxSize(dataPR, minchar=Param['minchar'], maxchar=Param['maxchar'], sepchar=Param['sepchar'])
...
#        out[alias[i]].iloc[getidx] = np.ones((2,len(getidx)),dtype=bool)
        if Param['showprogress']: sp.showprogress(i,len(alias),tictoc=toc())
    #end
