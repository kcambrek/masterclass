""" This function makes a HEATMAP using the input-dataset, including clustering if desired
 ...
 OPTIONAL
   ...
   cluster:        Boolean [0,1]: Cluster data hierarchical with euclediance distance and compolete linkage 
                   [1]: Yes
                   [0]: No (default)
...
#%%
def imagesc(data, labxcol='', labxrow='', cluster=0, norm=0, cmap='bwr', dpi=100, labxTop=1, xtickRot=45, ytickRot=0, width=10, height=10, savepath='', colorbar=1, distance='euclidean', linkage='ward', linewidth=0, caxis=[None,None], annot=False, plottype='default', showprogress=0):
	#%% DECLARATIONS
    out ={};
    fig ='';
    ...
    if Param['cluster']==1:
        ...
        # Cluster data
        g = sns.clustermap(data, method=Param['linkage'], metric=Param['distance'], col_cluster=True, row_cluster=True, cmap=Param['cmap'], standard_scale=None, z_score=None, figsize=(Param['width'], Param['height']))
#        g = sns.clustermap(data.corr(), center=0, cmap=Param['cmap'],row_colors=network_colors, col_colors=network_colors,linewidths=.75, figsize=(Param['width'], Param['height']))
        ...
        # Make heatmap
        ax = sns.heatmap(data, cmap=Param['cmap'], linewidths=Param['linewidth'], vmin=Param['caxis'][0], vmax=Param['caxis'][1], annot=Param['annot'])
        
#        sns.set_style("dark")

        # Set labels
        ax.set_xticklabels(labxcol, rotation=Param['xtickRot'], ha='center', minor=False)
        ax.set_yticklabels(np.flipud(labxrow), rotation=Param['ytickRot'], ha='right', minor=False)
